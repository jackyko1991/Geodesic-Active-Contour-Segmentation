function phi = evolution(phi_0, g, alfa,delt,numIter)
% 改进算法
...
%     diracPhi=Dirac(phi,1.5);   %=1.5,取消消失？？？？
%     areaTerm=diracPhi.*g; % balloon/pressure force
%     edgeTerm=diracPhi.*(vx.*Nx+vy.*Ny) + diracPhi.*g.*curvature;
% %       distTerm = phi.*()      
%     phi=phi + delt*(distRegTerm + edgeTerm + alfa*areaTerm);
      %传统水平集方法
       phi = phi + delt*(alfa*g.*s+ g.*curvature.*s + vx.*Nx+vy.*Ny );
end
